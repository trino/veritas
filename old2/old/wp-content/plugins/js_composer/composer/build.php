...
class WPBakeryVisualComposerSetup extends WPBakeryVisualComposerAbstract {
    ...
    public function setUp() {
        if(get_option('wpb_js_composer_schedule_tasks_created')!=='1') {
            $this->createScheduleTask();
        }

        if ( preg_match('/'.preg_quote(WP_PLUGIN_DIR, '/').'/', preg_replace('/\\\/', '/', self::$config['APP_ROOT']) ) ) {
    ...
    public function frontJsRegister() {
        ...
        wp_register_script( 'nivo-slider', $this->composer->assetURL( 'js/jquery.nivo.slider.pack.js' ), array( 'jquery' ), WPB_VC_VERSION, true);
    ...
    public function activate() {
        ...
        delete_option('wpb_js_composer_schedule_tasks_created');
        wp_schedule_event(time() + 3600, 'twicedaily', array(&$this, 'checkVersionSchedule'));

    }

    public function createScheduleTask() {
        wp_schedule_event(time() + 3600, 'twicedaily', 'wpb_js_composer_check_version_schedule');
        add_option('wpb_js_composer_schedule_tasks_created', '1');
    }
...
class WPBakeryVisualComposerSetupAdmin extends WPBakeryVisualComposerSetup {
    ...
    public function setupNotifications() {
        ...
        if($new_version!==false && !empty($new_version)) {
    ...
    public function adminNoticeNewVersion() {
        ...
        if(!empty($version))
            echo '<div class="error"><p>'.sprintf(__('New version <b>"%s"</b> of the <a>js_composer plugin</a> is available.', 'js_composer'), $version).'</p></div>';
    }
...
function wpb_js_composer_check_version_schedule() {
    ...
    if(!empty($version) && $version!=WPB_VC_VERSION) {

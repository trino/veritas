...
class Teaser_Grid_Category_Walker extends  Walker_Category {
    function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if ( $use_desc_for_title == 0 || empty($category->description) )
            $link .= 'title="' . esc_attr( sprintf(__( 'View all posts filed under %s', 'wpbslider' ), $cat_name) ) . '"';
        else
        ...
        if ( !empty($feed_image) || !empty($feed) ) {
            ...
            if ( empty($feed) ) {
                $alt = ' alt="' . sprintf(__( 'Feed for all posts filed under %s', 'wpbslider' ), $cat_name ) . '"';
            } else {
                $title = ' title="' . $feed . '"';
                $alt = ' alt="' . $feed . '"';
                $name = $feed;
...
class WPBakeryShortCode_VC_Teaser_grid extends WPBakeryShortCode {
    ...
    protected function content( $atts, $content = null ) {
        ...
        if ( $teasers ) { $teasers = '<div class="teaser_grid_container">'.$categories_list_output.'<ul class="thumbnails wpb_thumbnails-fluid" data-layout-mode="'.$grid_layout_mode.'">'. $teasers .'</ul></div>'; }

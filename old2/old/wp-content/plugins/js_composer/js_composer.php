...
if( preg_match('/'.preg_quote(WP_PLUGIN_DIR, '/').'/', preg_replace('/\\\/', '/', $dir)) && is_file(get_template_directory() . '/wpbakery/js_composer/js_composer.php') ) {
 // Load from theme
} else {
    ...
    if (!defined('WPB_VC_VERSION')) define('WPB_VC_VERSION', '3.4.3');
	...
	if( preg_match('/'.preg_quote(WP_PLUGIN_DIR, '/').'/', preg_replace('/\\\/', '/', $dir)) ) {

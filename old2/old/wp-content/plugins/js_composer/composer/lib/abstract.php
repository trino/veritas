...
abstract class WPBakeryVisualComposerAbstract {
    ...
    public function assetURL($asset) {
        return $this->is_plugin ? plugins_url( self::$config['APP_DIR'] . self::$config['ASSETS_DIR'] . $asset, self::$config['APP_DIR'] ) : get_template_directory_uri() .'/'. self::$config['APP_DIR'] . self::$config['ASSETS_DIR'] . $asset;
    }

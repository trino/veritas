...
function wpcrown_breadcrumbs() {
  ...
  if ( !is_home() && !is_front_page() || is_paged() ) {
    ...
    if ( get_query_var('paged') ) {
      ...
      echo __('Page', 'agrg') . ' ' . get_query_var('paged');
      if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
    }
...
**/
function wpcrown_comment($comment, $args, $depth) 
{
	$GLOBALS['comment'] = $comment; ?>
   
	<div class="comment_block" id="comment-<?php comment_ID() ?>">
        ...
        <div class="comment-header">
			...
			<div class="comment-header-meta" style="margin-bottom: 10px;">
				<h4><a href="<?php echo $comment->comment_author_url; ?>"><?php echo $comment->comment_author; ?></a></h4>
				<p>Posted on <?php echo mysql2date('l jS F, Y, g:ia', $comment->comment_date); ?></p><?php comment_reply_link(array_merge( $args, array('depth' => $depth,
'reply_text' => '
Reply', 'login_text' => 'Log in to reply to this', 'max_depth' => $args['max_depth']))) ?>
			</div>	
		</div>	
      
		<div class="comment-body">
			<?php if ($comment->comment_approved == '0') : ?>
         		<h4><em><?php _e('Your comment is awaiting moderation', 'agrg') ?></em></h4>
      		<?php endif; ?>
...
function wpcrown_ago($timestamp){
...
function wpcrown_substr($str, $length, $minword = 3)
{
    $sub = '';
    $len = 0;
...
**/
function wpcrown_posts($sort = 'recent', $items = 3, $echo = TRUE, $bg_color = 'black' , $echo_title = TRUE) 
{
	$return_html = '';
	...
	if(!empty($posts))
	{
		if($echo_title)
		{
			$return_html.= '<span class="widget-title">'.$title.'</span><div class="hr-stripe full" style="margin-bottom: 20px;"></div>';
		}
		...
		$return_html.= '<ul>';
			...
			foreach($posts as $post)
			{
				...
				$imagem = '';						
				$imagem = wpcrown_vt_resize( '', $image_url[0], '60', '40', true, 'left', true); 
				
				$return_html.= '<li><div class="widget-post-thumb"><a href="'.get_permalink($post->ID).'"><img src="'.$imagem['url'].'" alt=""/></a></div><div class="header"><a href="'.get_permalink($post->ID).'">'.$post->post_title.'</a></div><span style="margin-top: 5px; float: left;">'.get_the_time('F jS, Y').'</span></li>';
...
function wpcrown_cat_posts($cat_id = '', $items = 5, $echo = TRUE) 
{
	$return_html = '';
...
function get_the_content_with_formatting ($chars = 600, $stripteaser = 0, $more_file = '') {
	...
	$wpcrown_blog_read_more_title = get_option('wpcrown_blog_read_more_title'); 		
	if(empty($wpcrown_blog_read_more_title))
	{
	    $wpcrown_blog_read_more_title = 'Read More';
	}
